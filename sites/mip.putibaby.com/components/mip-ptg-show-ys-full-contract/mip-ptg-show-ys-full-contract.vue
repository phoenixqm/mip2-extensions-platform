...
<div class='paidan body'>
  ...
  <div v-if="data.is_admin" class='order_number'>
    <p>
  ...
  <div class='sign_table' v-if="data.is_admin">
...
export default {
  ...
  data() {
    ...
    var xsrhl = ys_skills.filter(function(x) {
      return x.title == '新生儿护理';
    })[0].list.map(function(x) {
      return x.title;
    }).join('，');

    var yrhl = ys_skills.filter(function(x) {
      return x.title == '婴儿护理';
    })[0].list.map(function(x) {
      return x.title;
    }).join('，');

    var qtfw = ys_skills.filter(function(x) {
      return x.title == '其他服务';
    })[0].list.map(function(x) {
      return x.title;
    }).join('，');
